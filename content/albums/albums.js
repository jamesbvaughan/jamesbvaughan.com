...
var author$project$Main$GotAlbums = function (a) {
	return {$: 'GotAlbums', a: a};
};
var author$project$Main$Album = F3(
	function (title, uri, imageUrl) {
		return {imageUrl: imageUrl, title: title, uri: uri};
	});
...
var author$project$Main$albumDecoder = A4(
	...
	A2(elm$json$Json$Decode$field, 'display_title', elm$json$Json$Decode$string),
...
var author$project$Main$responseDecoder = A2(
	...
	'items',
	elm$json$Json$Decode$list(author$project$Main$albumDecoder));
...
var author$project$Main$fetchAlbums = elm$http$Http$get(
	{
		expect: A2(elm$http$Http$expectJson, author$project$Main$GotAlbums, author$project$Main$responseDecoder),
		url: 'https://api.discogs.com/lists/447073?token=PsodRNmqSCATJYchOwiGuaNrxArwuUJxFOyHwsDG'
	});
var author$project$Main$init = function (_n0) {
	return _Utils_Tuple2(author$project$Main$Loading, author$project$Main$fetchAlbums);
};
var author$project$Main$Failure = {$: 'Failure'};
var author$project$Main$ShuffledAlbums = function (a) {
	return {$: 'ShuffledAlbums', a: a};
};
		...
		if (msg.$ === 'GotAlbums') {
			...
			if (result.$ === 'Ok') {
				...
				return _Utils_Tuple2(
					...
					A2(
						elm$random$Random$generate,
						author$project$Main$ShuffledAlbums,
						elm_community$random_extra$Random$List$shuffle(albums)));
			} else {
			...
			var albums = msg.a;
			return _Utils_Tuple2(
				author$project$Main$Success(albums),
...
var author$project$Main$albumView = function (album) {
	return A2(
		elm$html$Html$li,
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('w-third-ns w-50')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$href(album.uri)
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$src(album.imageUrl),
								elm$html$Html$Attributes$class('mw-100')
							]),
						_List_Nil)
					]))
			]));
	...
	switch (model.$) {
		case 'Failure':
			return A2(
				...
				_List_fromArray(
					[
						elm$html$Html$text('hey it failed')
					]));
		case 'Loading':
			return A2(
				elm$html$Html$p,
		...
		default:
			...
			return A2(
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$id('album-list'),
						elm$html$Html$Attributes$class('list flex flex-wrap justify-around pa0'),
						A2(elm$html$Html$Attributes$style, 'font-size', '0px')
					]),
				A2(elm$core$List$map, author$project$Main$albumView, albums));
	}
};
